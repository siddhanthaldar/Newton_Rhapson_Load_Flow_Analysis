...
for x=1:nbs   %row
    for y=x:nbs
       if x==y
           for k=1:n_lines
               if (line_dat(1,k) == x)
                   z = line_dat(3,k)+j*line_dat(4,k);
                   Ybus(x,y) = Ybus(x,y) + (1/line_dat(6,k)^2)*(1/z + line_dat(5,k)/2);
               elseif (line_dat(2,k) == x)
                   z = line_dat(3,k)+j*line_dat(4,k);
                   Ybus(x,y) = Ybus(x,y) + (1/z + line_dat(5,k)/2);
               end
           end
       else 
           for k=1:n_lines
               if (line_dat(1,k) == x) || (line_dat(2,k) == y)
                   z = line_dat(3,k)+j*line_dat(4,k);
                   Ybus(x,y) = Ybus(x,y) + (-1/line_dat(6,k))*(1/z + line_dat(5,k)/2);
...
for i=1:nbs
    ...
    if bus_dat(2,i) == 101
        ...
        for it=1:nbs
            ...
            Q(1,i) = Q(1,i) + bus_dat(3,it)*bus_dat(3,it)*(re*sin(ang)-img*cos(ang));
        end
...
for x=1:dim-(nbs-1)
    for y=1:nbs-1
        if x==y
            J(x+nbs-1,y) = P(1,x)- bus_dat(3,x)*bus_dat(3,y)*imag(Ybus(x,y));
        else
            ang = bus_dat(4,x)-bus_dat(4,y);
            re = real(Ybus(i,it));
            img = imag(Ybus(i,it));
